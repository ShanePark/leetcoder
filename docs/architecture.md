...
`src/app/project-search-controller.ts` adds content results to the file explorer
after its immediate filename filter. It debounces queries and runs one search at
a time through `src-tauri/src/project_search.rs`, which reads saved text without
maintaining an index. The app connects editable problem-file results to document
navigation; other text results remain previews in the sidebar.
...
| Backend response adapters | `src/backend/normalizers/{common,problem,files,git,test,project-search}.ts` | Convert native and older response shapes into frontend contracts. `common.ts` owns shared guards and primitive coercions. | Backend contracts and common helpers |
...
| Project search | `src/app/project-search-controller.ts`, `project-search-location.ts`, `src/styles/project-search.css`, `src-tauri/src/project_search.rs` | Append bounded literal content results to filename searches, locate clicked results in the current source, and scan project text while skipping generated folders, binary files, oversized files, and symlinks. | Backend contracts, app navigation callbacks, native models and security |
